...
def build_rocketsim_env():
    ...
    reward_fn = CombinedReward.from_zipped(
	    (EventReward(touch=1, team_goal=4, concede=-4), 5), # Reward for getting goals and touching the ball
        (VelocityBallToGoalReward(), 7), # Move the ball towards the goal
	    (SpeedTowardBallReward(), 3), # Move towards the ball!
...
if __name__ == "__main__":
    ...
    wandb_group_name = "ducky-basic-score"
    wandb_run_name = "ducky-basic-score-run"

    # Save/load path setup
    add_unix_timestamp = False;
    checkpoints_save_folder = os.path.join("data", "checkpoints", wandb_group_name, wandb_run_name)
    
    #ballchase_save_folder = os.path.join("data", "checkpoints", "ducky-ballchase", "ducky-ballchase-run")# 
    #latest_checkpoint_dir = os.path.join(ballchase_save_folder, str(max(os.listdir(ballchase_save_folder), key=lambda d: int(d))))
